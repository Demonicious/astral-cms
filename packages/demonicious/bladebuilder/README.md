...
$builder = new BladeBuilder\BladeBuilder($config);
...
## Extending BladeBuilder

BladeBuilder allows you to create new blocks and layouts for your theme very easily. This is great for building specific websites & templates, however if you use BladeBuilder in a CMS environment, you probably want to provide Plugins / Modules the ability to create their own blocks without modifying your theme's pre-existing components.

BladeBuilder allows you to register blocks, layouts and assets (CSS, JS) from Plugins, Composer Packages or through any other environment.
 ...
 */
\BladeBuilder\Extensions::registerBlock($slug, $directoryPath);

// Registering an example block:

\BladeBuilder\Extensions::registerBlock('foo-navbar', MY_PLUGIN_DIRECTORY . '/blocks/foo-navbar');
```
 ...
 */
\BladeBuilder\Extensions::registerLayout($slug, $directoryPath);

// Registering an example layout:

\BladeBuilder\Extensions::registerLayout('foo-master', MY_PLUGIN_DIRECTORY . '/layouts/foo-master');
```
 ...
 */
\BladeBuilder\Extensions::registerAsset($src, $type, $location, $attributes = []);

// Registering assets:

\BladeBuilder\Extensions::registerAsset('/assets/bootstrap.css', 'style', 'header');
\BladeBuilder\Extensions::registerAsset('/assets/alpine.min.js', 'script', 'header', [
    'defer' => true
]);
```
...
#### Disable the module
Do you already have admin login functionality in your project? Then you can disable the website manager module by setting  `use_website_manager` to `false` in your config. Next, you use or implement the page create/edit/remove functionality in your project and then directly launch the pagebuilder. You can render the pagebuilder from your project by using the `BladeBuilder\Modules\GrapesJS\PageBuilder` class.
